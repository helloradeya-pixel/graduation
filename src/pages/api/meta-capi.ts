...
export default async function handler(
  ...
  try {
    ...
    console.log(`🔥 CAPI HIT [v20.0] | Event: ${event_name} | EventID: ${event_id}`);
    ...
    // =========================
    // SEND TO META (Dua Pixel Sekaligus)
    // =========================
    // Pixel Lama dari .env (804715912719122) dan Pixel Baru Frame (1413881487242621)
    const pixelIds = [process.env.PIXEL_ID, '1413881487242621'];
    ...
    const results = await Promise.all(
      pixelIds.map(async (pixelId) => {
        ...
        return { pixelId, data: await response.json() };
      })
    );

    return res.status(200).json({
      success: true,
      debug: { event_name, event_id },
